...
import { MoreHorizontal, FilePenLine, Trash2, UserPlus, Building, Loader2, PlusCircle, Mail, User, Shield } from 'lucide-react';
...
const UserActions = ({ user, onEdit, onDelete }: { user: User, onEdit: (user: User) => void, onDelete: (id: number) => void }) => (
    <AlertDialog>
        <DropdownMenu>
...
const DepartmentActions = ({ dept, onEdit, onDelete }: { dept: Department, onEdit: (dept: Department) => void, onDelete: (id: number) => void }) => (
    <AlertDialog>
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="h-8 w-8 p-0">
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                <AlertDialogTitle>Are you sure?</AlertDialogTitle>
                <AlertDialogDescription>This action cannot be undone and will permanently delete the department.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                ...
                <Card>
                    <CardHeader>
                        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                            ...
                            <Button onClick={handleNewUser} className="w-full md:w-auto"><UserPlus className="mr-2 h-4 w-4" /> Add User</Button>
                        </div>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                            ...
                            <Button onClick={handleNewDepartment} className="w-full md:w-auto"><Building className="mr-2 h-4 w-4" /> Add Department</Button>
                        </div>
                    </CardHeader>
                    <CardContent>
                        {loading ? <Loader2 className="mx-auto h-8 w-8 animate-spin" /> : (
                                        ...
                                        <Card key={dept.id}>
                                            <CardHeader>
                                                 <div className="flex justify-between items-start">
                                                    ...
                                                    <DepartmentActions dept={dept} onEdit={handleEditDepartment} onDelete={handleDeleteDepartment} />
                                                 </div>
                                            </CardHeader>
                                        </Card>
                                    ))}
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            ...
                                            <TableRow key={dept.id}>
                                                ...
                                                <TableCell>{dept.supervisor?.name ?? <span className="text-muted-foreground italic">None</span>}</TableCell>
                                                <TableCell className="text-right">
                                                    <DepartmentActions dept={dept} onEdit={handleEditDepartment} onDelete={handleDeleteDepartment} />
                                                </TableCell>
                                            </TableRow>
